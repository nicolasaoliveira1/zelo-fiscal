...
- **Freio automático quando um portal cai.** Três falhas seguidas no mesmo portal abrem um *circuit breaker*: o lote daquele portal para (em estado retomável) e um alerta sai por e-mail, em vez de o sistema queimar créditos de solver contra um site fora do ar. Se o portal for municipal, só ele para — os outros municípios seguem emitindo. O bloqueio expira sozinho, então um portal que volta de madrugada não fica esperando alguém religar nada. E o que morreu na fila não some: as emissões que esgotaram as tentativas aparecem agrupadas por motivo no painel de diagnóstico e voltam à fila com um clique, sem duplicar.
